"""
Vosk Accuracy Test with Medical Keyword Detection
Uses Vosk model to transcribe audio and match against medical keywords
"""
...
# Configuration
VOSK_MODEL = "vosk-model-small-en-us-0.15"  # Small Vosk model - faster loading
SAMPLE_RATE = 16000
...
VOSK_MODEL_PATH = os.path.join(PROJECT_ROOT, "raspi_system", "vosk_model", VOSK_MODEL)
...
class AudioRecorder:
    ...
    def record(self):
        """Record audio from microphone and return int16 audio."""
...
class VoskTranscriber:
    ...
    def _load_model_async(self):
        ...
        try:
            self.loading = True
            print(f"[EXEC] Loading Vosk model from {self.model_path} in background...")
            ...
            print("[EXEC] Check that the model path is correct")
            self.load_event.set()
    ...
    def wait_for_model(self, timeout=MODEL_LOAD_TIMEOUT):
        ...
        if not self.load_event.wait(timeout=timeout):
            raise TimeoutError(
                ...
                f"Check your disk space and model path."
            )
...
def get_model_info():
    ...
    return jsonify(
        {"model_type": "Vosk", "model_name": VOSK_MODEL, "model_path": VOSK_MODEL_PATH}
    )
...
def run_single_test():
    ...
    try:
        ...
        if collected:
            audio_data = np.concatenate(collected).astype(np.int16)
            print(
                "[TEXT] Transcribing complete audio with Vosk... (this may take a moment)"
            )
...
def print_summary():
    ...
    results_path = os.path.join(SCRIPT_DIR, "test_results_vosk.json")
...
def generate_excel_report():
    ...
    filename = f"test_results_vosk_{timestamp}.xlsx"
    ...
    for idx, result in enumerate(transcription_results, 2):
        ...
        ws.cell(row=idx, column=8, value=result.get("model_type", "Vosk"))
        ws.cell(row=idx, column=9, value=result.get("model_name", VOSK_MODEL))
...
if __name__ == "__main__":
    try:
        print("=" * 60)
        print("VOSK ACCURACY TEST - MEDICAL KEYWORD DETECTION")
        print("=" * 60)
        print("\nConfiguration:")
        print(f"  Model: Vosk {VOSK_MODEL}")
        ...
        if not os.path.exists(VOSK_MODEL_PATH):
            ...
            print(
                "[EXEC] Please ensure the model is downloaded and in the correct location"
            )
            sys.exit(1)
        ...
        print("\n[EXEC] Waiting for Vosk model to load...")
    ...
    print("\n[OK] Test completed. Results saved to test_results_vosk.json")
